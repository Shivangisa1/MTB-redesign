...
test.describe('Validating the Searching Machines ', () => {

    test.beforeEach(async ({ page }) => {

        // monitorAPI(page); check for performance-API test for each test case
        ...
        // await logPageLoad(page);  check for performance-Pageload test for each test case

    });

    //-----------------------------IN PROgresss------------------------------------
    test.fail('Test Case : View Details-- Verify that all contact us buttons working', async ({ page }) => {

        await page.getByRole('link', { name: 'View Details' }).first().click();
        await page.waitForTimeout(2000);
        const count1 = await page.getByRole('link', { name: 'Contact Us' }).first().click();
        console.log("no close on modal window or either slow response")

        const modal = await page.locator('.fade contact-us');
        await modal.getByPlaceholder('First Name ').fill('test');

    });


    test('Test Case 1 : Verify the Total number of Filters to find the machines in Machine Tool Bids', async ({ page }) => {
    ...
    test('Test Case 2 : Verify the Filters Name to find the machines in Machine Tool Bids', async ({ page }) => {
        const filter = await page.locator('.filter-dropdown').locator('h5');
        const count = await filter.count();
    ...
    test('Test Case 6 : Verify that the user can select both Brand name, machine type & check results', async ({ page }) => {
        ...
        await page.getByRole('heading', { name: 'Brand' }).click();
        await page.getByLabel('Toshiba').check();
        await page.waitForTimeout(2000);
    ...
    test.skip('Test Case 8 : Verify that the user can select year from the filter & check results', async ({ page }) => {
    ...
    test('Test Case 10 : Verify that the user can basic details of selected machine', async ({ page }) => {
        await page.getByRole('link', { name: 'View Details' }).first().click();
        const container = page.locator('.machine-detail');

        await page.waitForTimeout(2000);
        // Offer Price
       const offerPrice = await container.locator('h3.amount a').first();     
        ...
        async function getField(labelText: string) {
            const label = container.locator(`label:has-text("${labelText}")`).first();
